...
class Form extends Component {
    ...
    getItem(){
        axios.get(cfg.baseURL+'api/chapter/'+this.state.id)
        .then((response) => {
            let formValues = this.state.formValues;
            formValues.title = response.data.title;
            formValues.book_title = response.data.book_title;
            formValues.authors = response.data.authors;
            formValues.year = response.data.year;
            formValues.city = response.data.city; 
    ...
    updateItem(data, id){
        axios.put(cfg.baseURL+'api/chapter/'+id, data)
            .then(alert('Updated'))
    ...
    createItem(data){
     ...
     handleSubmit(e) {
        ...
        const data = {
            ...
            book_title: this.state.formValues.book_title,
                ...
                <div className="row">
                    <div className="col-md-12">
                        <div className="form-group">
                            ...
                            required="required" data-error="Authors are required."
                            onChange={ this.handleInputChange }
                            value={ this.state.formValues.book_title }   />
                        <div className="help-block with-errors"></div>
                        </div>
                    </div>                                
                </div>
